...
{ a b } : unpack-input over Locals| rootx | yindex ! xindex !
	input-size @ 0 do 
	holding-loc @ I + c@ { find value in unpacking array }
		case
			48 of 0 xindex @ yindex @ L_ARRAY_! xindex @ 1+ xindex ! endof
			49 of lives @ xindex @ yindex @ L_ARRAY_! xindex @ 1+ xindex ! endof
			13 of rootx xindex ! yindex @ 1 - yindex ! endof
		endcase
	loop ." input unpacked " ;
...
: load-from-file loadinput finddims gridx @ swap - 2 / swap gridy @ swap - 2 / unpack-input ;
...
: initialise_fromfile full-load-from-file find_neighbours_2 1 stepcounter ! ;
: initialise_withfile create_arrays load-from-file find_neighbours_2 1 stepcounter ! ;
{ full initialisation codes to be run at the start of any routine }

{ a } : run_preset_no_display cr ." Running preset simulation without display for " 
	dup . ." generations "
